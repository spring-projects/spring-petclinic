    ...
    stages {
        ...
        stage('Commit message checkuot') {
            steps {
                ...
                sh 'if [[ $(echo ${GIT_COMMIT_MSG} | grep -qP "^[A-Z]+-[0-9]+[\w ,.:-\"\']{,72}\n+([\w ,.:-\"\']{,80}\n)*" ; echo $?) == 0 ]]; then echo "Commit message checkout passed!"; else echo "Commit message doesnt complies with best practices. See: https://robertcooper.me/post/git-commit-messages"; exit 1'
            }
        }
