...
To see the solutions for each task, go to the `practical-task` folder. Each task has it's own subfolder (named according to the task number) with a README file. Also check the commit history for file modifications.
